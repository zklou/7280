...
class Dendrogram(nx.DiGraph):
    ...
    def __init__(self, G=None):
        ...
        if G is not None:
            ...
            self.graph_nodes_list = list(G.nodes())
    ...
    def initialize_graph_keys(self):
        ...
        self.graph_nodes_list = list(self.G.nodes())
        self.graph_nodes_list.sort()
    ...
    def initialize(self, layout="random"):
        ...
        # child graph node.
        sorted_graph_nodes = list(self.G.nodes())
    ...
    def enforce_order_property(self, node):
        ...
        if node_label > left_child_label:
            s = str(self)
            raise Exception(
                "order property violated by node "
                + str(node)
                + " "
                + "label "
                + str(node_label)
    ...
    def left_beta_move(self, child, update_leaf_nodes=True):
        ...
        if (
            self.nodes[child_right_child]["orderprop"]
            < self.nodes[child_right_child]["orderprop"]
        ):
            self.remove_left_child(child, child_left_child)
            self.remove_right_child(child, child_right_child)
    ...
    def update_numbers_of_leaf_nodes(self, parent):
        ...
        if self.is_graph_node(left_child):
            ...
            left_count += self.get_number_of_leaf_nodes(left_child, "L")
            left_count += self.get_number_of_leaf_nodes(left_child, "R")
        ...
        if self.is_graph_node(right_child):
            ...
            right_count += self.get_number_of_leaf_nodes(right_child, "L")
            right_count += self.get_number_of_leaf_nodes(right_child, "R")
        ...
        for i, gnode in enumerate(self.graph_nodes_list):
            if gnode in C and gnode in M:
                raise Exception(
                    ...
                    + str(gnode)
                    + " exists on "
                    + "both sides of the split for dendrogram node"
                    + str(dnode)
                )
                ...
                raise Exception(
                    ...
                    + str(gnode)
                    + " is not on "
                    + "either side of the split for dendrogram node"
                    + str(dnode)
                )
    ...
    def plot(self):
        ...
        dendrogram = sch.dendrogram(
            L, labels=labels, color_threshold=0, link_color_func=lambda x: "0.0"
        )
        plt.gray()
    ...
    def linkage(self, root="_D0"):
        ...
        # the root).
        state = {"label-counter": self.number_of_graph_nodes, "probabilities": []}
...
class ConsensusDendrogramBuilder(object):
    ...
    def build(self, graph_nodes, histograms, num_samples):
        ...
        if type(histograms) == list:
            ...
            raise Exception("unknown type of histograms " + str(type(histograms)))
    ...
    def consensus_tree_as_string(self, consensus):
        ...
        for cnode, d in consensus.nodes(data=True):
            ...
            s += str([cnode, str(weight), str(len(successors)), str(successors)])
            s += "\n"
